...
class TestIMDbDatabase(TestCase):
    ...
    # since IMDb is now in the test_imdb (this file) namespace, we need to patch
    # the test_imdb.IMDb.search_titles method
    ...
    # Mock: 200
    @patch('models.imdb.requests.get')
    def test_seach_by_title_failed(self, imdb_mock):
        """Test searching by title failed"""
        imdb_mock.return_value = Mock(
            spec=Response,
            status_code=200,
            json=Mock(
                return_value=IMDB_DATA["INVALID_API"]
            )
        )
        imdb = IMDb("bad-key")
        results = imdb.search_titles("Bambi")
    ...
    def test_movie_ratings(self, imdb_mock):
        ...
        imdb_mock.return_value = Mock(
            ...
            status_code=200,
            json=Mock(
                return_value=IMDB_DATA["GOOD_RATING"]
            )
        )
        imdb = IMDb("k_12345678")
        results = imdb.movie_ratings("tt1375666")
        self.assertIsNotNone(results)
        self.assertEqual(5, results["imDb"])
        self.assertEqual(4, results["metacritic"])
        self.assertEqual(4, results["theMovieDb"])
        self.assertEqual(5, results["imDb"])
